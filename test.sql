...
FROM table_with_url;
VACUUM ANALYSE ;
EXPLAIN ANALYSE SELECT my_url FROM table_with_url WHERE sameFile(my_url, 'httpsx://www.norse.be.super:4242/duc/');
...
EXPLAIN ANALYSE SELECT my_url from table_with_url where samePath(my_url, 'httpsx://www.norse.be.super:4242/duc/');
EXPLAIN ANALYSE SELECT my_url from table_with_url where samehost(my_url, 'httpsx://gaspardmertenpremierdunometavecuntreslongnom:4242/duc/');
EXPLAIN ANALYSE SELECT my_url from table_with_url where equals(my_url, 'httpsx://www.norse.be.super:4242/duc/');
